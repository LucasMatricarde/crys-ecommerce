# order-read-model Specification

## Purpose

Defines the CQRS read side: order-query-service consumes order, inventory, and payment events to maintain a denormalized `order_view` read model in Mongo and exposes query endpoints over it.
