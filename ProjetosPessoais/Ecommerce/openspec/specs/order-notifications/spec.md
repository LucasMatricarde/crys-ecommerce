...
Defines notification-service, which consumes `order.events` and persists a `notification` document per relevant lifecycle event with mock delivery and on-brand pt-BR copy, plus a query endpoint to list an order's notifications.
