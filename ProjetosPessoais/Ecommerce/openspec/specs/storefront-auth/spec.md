# storefront-auth Specification

## Purpose

Defines the storefront's demo authentication: a demo-only login that mints a JWT, a local token store with an HTTP authorization interceptor for protected endpoints, route guards for authenticated views, and 401-driven session clearing.
