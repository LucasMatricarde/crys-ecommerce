...
Define how the CRYS stack is packaged and deployed to a Kubernetes cluster via a single Helm chart — covering single-command full-stack install, DRY service templating, container health probes, env-driven in-cluster configuration, in-cluster infra parity with docker-compose, image build/load tooling, and a live end-to-end order saga on the cluster.
