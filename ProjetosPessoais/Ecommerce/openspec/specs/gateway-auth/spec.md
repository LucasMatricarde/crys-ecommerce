...
Defines JWT-based authentication enforced at the API gateway: public routes that bypass auth, protected routes requiring a valid HS256-signed bearer token, downstream identity forwarding via `X-User-Id`, and a demo token-mint endpoint standing in for a real identity provider.
