...
Defines end-to-end distributed tracing across the CRYS microservices: the api-gateway records and propagates W3C trace context over HTTP, the transactional outbox persists the active trace context at write time, the relay re-injects it as Kafka headers, saga listeners continue the trace from those headers, and a single order appears in Jaeger as one connected trace under a single traceId.
