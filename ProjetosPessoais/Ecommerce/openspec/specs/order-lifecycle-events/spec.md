# order-lifecycle-events Specification

## Purpose

Defines the terminal lifecycle events that order-service emits from the saga (`OrderConfirmed`, `OrderCancelled`) onto `order.events`, and the shared event contract in `libs/common-events` that producers and consumers rely on.
