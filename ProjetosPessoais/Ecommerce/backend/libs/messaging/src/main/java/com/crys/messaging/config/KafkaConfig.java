...
/**
 * Kafka wiring shared by all saga services. Envelopes travel as JSON strings
 * (already serialized into the outbox), so String (de)serializers are used.
 * A {@link DefaultErrorHandler} retries a failing record a few times, then a
 * {@link DeadLetterPublishingRecoverer} routes it to {@code <topic>.DLT}.
 */
@Configuration
